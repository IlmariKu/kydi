...
export function SVGStar(props) {
  function getStar(starcount = 1) {
    const stars = [];

    _.range(starcount).forEach(function (starCount) {
      stars.push(
        <svg height="1em" key={`star_${starCount}`} viewBox="0 -10 511.987 511" width="1em" {...props}>
          <path
  ...
  return <div>{getStar(props.starcount)}</div>;
}
